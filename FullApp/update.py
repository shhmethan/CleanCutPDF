...
VERSION_INFO_URL = (
    ...
    "shhmethan/CleanCutPDF/master1/version.json"
)
...
def write_update_log(message):
    ...
    try:
        ...
        with open(
            ...
            file.write(
                f"[{timestamp}] Update: {message}\n"
            )

    except Exception as error:

        print(
            f"Could not write updater log: {error}"
        )
...
def get_theme_config(
    ...
    if (
        theme_name == "Dark Pink"
        and PINK_DARK.exists()
    ):
...
# ─────────────────────────────────────────────
# WAIT FOR CLEAN CUT PDF TO CLOSE
# ─────────────────────────────────────────────
...
def wait_for_process(
    ...
    try:
        ...
        print(
            f"Could not wait for process: {error}"
        )
...
class CleanCutPDFUpdater(
    ...
    def build_ui(
        ...
        self.updater_label = ctk.CTkLabel(
        ...
        self.progress_container.pack_propagate(
            False
        )
        ...
        self.auto_launch_checkbox.pack(
            pady=(4, 12)
        )
    ...
    def check_worker(
        ...
        try:
            ...
            self.download_url = data.get(
                "download_url"
            )


            self.release_notes = data.get(
    ...
    def start_update(
        ...
        if not self.app_path:
            ...
            messagebox.showerror(
                ...
                (
                    "The updater was not given "
                    "the CleanCutPDF application "
                    "path."
                )
            )
        ...
        self.auto_launch_checkbox.configure(
            state="disabled"
        )
            ...
            try:
                ...
                if (
                    ...
                    write_update_log(
                        ...
                        f"Restored: {self.current_version}"
                    )
